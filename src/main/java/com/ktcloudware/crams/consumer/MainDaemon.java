...
public class MainDaemon implements Daemon {
	 ...
	 * load plugins // TODO Fixme
	 * 
	 * List<CramsIndexerPlugin> plugins = new ArrayList<CramsIndexerPlugin>();
	 * if(topic.endsWith("usage")){ plugins.add(new DiskUsageCachePlugin());
	 * 
	 * } else if(topic.endsWith("rrd")){ plugins.add(new
	 * FiveMinutesFilterPlugin()); plugins.add(new DateFormatPlugin());
	 * plugins.add(new AppendDiskUsagePlugin()); plugins.add(new
	 * CpuAvgPlugin()); plugins.add(new MemoryUsagePlugin()); plugins.add(new
	 * VbdReadWriteAvgPlugin()); plugins.add(new VifAvgPlugin()); }
	 * 
	 * return plugins; }
	 */
}
