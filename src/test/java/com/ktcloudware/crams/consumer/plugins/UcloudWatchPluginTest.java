...
public class UcloudWatchPluginTest {
	...
	public void test() {
		...
		for (int i = 0; i < testDataSize; i++) {
			...
			String dayAndMonth = String.valueOf(year) + "-" + String.valueOf(month + 1) + "-" + String.valueOf(day - 5);
			int hour = i % 60;
			int minute = i / 60;
			data.put("datetime",dayAndMonth + " " + hour + ":" + String.valueOf(minute) + ":40");
	...
	public void testWrongDateFormat() {
		...
		for (int i = 0; i < testDataSize; i++) {
			...
			int hour = i % 60;
			int minute = i / 60;
			data.put("datetime",dayAndMonth + "T" + hour + ":" + String.valueOf(minute) + ":40");
			data.put("vif_0_rx", 465025100);
			data.put("vif_0_tx", 203703075);
			data.put("vbd_hda_read", 47568770);
			data.put("vbd_hda_write", 879123556);
